-- Restore profile photos clobbered by the Google-login overwrite (ZYR-1137)
--
-- WHY THIS EXISTS
--   LoginGoogle used to assign tb_user.image_upload = <google picture claim>
--   unconditionally on every sign-in, so a photo the user had uploaded was
--   replaced by their Google avatar. That path never called deleteOldAvatar, so
--   the uploaded object is still in the bucket — these 20 accounts are
--   recoverable without asking anyone to upload again.
--
-- HOW THE LIST WAS BUILT (2026-09-01, read-only)
--   1. gcloud storage ls -l gs://zyra-prod-gather-dev-458614/profiles/**
--   2. per user, take the NEWEST non-thumb object — that is the photo they last
--      chose (predecessors accumulated because the delete never ran)
--   3. keep only users whose tb_user.image_upload is currently a
--      googleusercontent URL, i.e. still clobbered
--   The bucket held 44 full-size objects across 28 users; the other 8 users
--   still point at their own photo and are deliberately NOT in this list.
--
-- !! ORDER MATTERS - DO NOT RUN THIS BEFORE THE FIX IS LIVE ON PROD !!
--   The fix is zyra-api commit b1ab1a7 (PR #58), currently on develop only.
--   Until it ships to prod (develop -> main -> tag v*), each of these users is
--   re-clobbered on their very next Google sign-in, so running this now buys
--   nothing. Ship the API release first, then run this.
--
-- SAFE TO RE-RUN
--   The WHERE clause only touches rows STILL holding a googleusercontent URL, so
--   anyone who has uploaded a new photo since the list was built keeps theirs.
--
-- HOW TO RUN
--   cd zyra-service && ./prod-db.sh up
--   ./prod-db.sh file ../zyra-doc/ops/restore-clobbered-profile-photos-2026-09-01.sql --write
--
-- DRY RUN FIRST (read-only, prints exactly which rows would change):
--   ./prod-db.sh file ../zyra-doc/ops/restore-clobbered-profile-photos-2026-09-01-dryrun.sql
--   Verified against prod on 2026-09-01: returns 20 rows. It must return 0 rows
...
UPDATE tb_user u
   SET image_upload = r.photo_url,
       updated_at   = NOW()
  FROM restore r
 WHERE u.id = r.user_id
   -- never overwrite a photo the user uploaded after the list was built
   AND u.image_upload LIKE '%googleusercontent%';

COMMIT;

-- ── Verification after running ───────────────────────────────────────────────
-- The companion dry-run file must return 0 rows:
--   ./prod-db.sh file ../zyra-doc/ops/restore-clobbered-profile-photos-2026-09-01-dryrun.sql
--
-- Whole-table check:
--   SELECT count(*) FILTER (WHERE image_upload LIKE '%storage.googleapis.com%') AS own_photos,
--          count(*) FILTER (WHERE image_upload LIKE '%googleusercontent%')      AS google_pics
--     FROM tb_user;
--   own_photos: 8 -> 28.  google_pics: 121 -> 101.
