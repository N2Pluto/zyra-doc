...
| [\[Feature\] Nature Object Management](<[Feature] Nature Object Management — Object Management/>) | SC-OBJ-NAT-01 — เพิ่ม Object Type "Nature" (ต้นไม้/พุ่มไม้/ดอกไม้) ใน Object Management (Admin) เดิม — animated spritesheet ต่อ animation state, weather-reactive intensity, walkable เสมอ — spec + technical design (schema `tb_object_animation`, API contract, realtime broadcast) + ux-ui-plan (ถอด Figma ครบ 8 section: 3 modal ใหม่ + reuse map + 35 จุดขัด spec) + test-plan + progress | **Planning only — ยังไม่ implement** · 2026-09-17 · ClickUp [86d446fw1](https://app.clickup.com/t/86d446fw1) · **รอบที่ 3: status ทุก task `pending` → `in progress` แต่ description/comment ไม่เปลี่ยนเลย** = เปิดงานแล้วแต่ spec ยังไม่ครบ · naming ยืนยันแล้ว (`shedding_tree`/`falling`) · HP-04 **descoped** · **Figma ขัด spec 12 ข้อใหญ่ที่ยังเปิดครบทุกข้อ** (required = idle เท่านั้น, ไม่มี frame_count input, ไม่มี Nature menu, ไม่มี preview controls, Status default Active, สี/ประเภท box vs always-walkable…) → ต้องเคลียร์ ux-ui-plan §14.1 กับ PM ก่อนแตะ migration |
