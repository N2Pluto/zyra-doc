...
|---|---|---|
| [Analytics](Analytics/) | Business events (GA4 via GTM + Mixpanel) — event catalog 7 กลุ่ม (auth, workspace, VO, chat, pet, settings, landing), naming/PII rules, destinations, task breakdown 10 task ~2–3 วัน · ต่อยอดจาก guides/analytics-monitoring-pwa.md | **Planning only — ยังไม่ implement** · 2026-09-09 · รอตอบ open questions 4 ข้อ |
...
| [\[Feature\] · Environment (Time of Day + Weather)](<[Feature]  · Environment (Time of Day + Weather) — Virtual Office Map/>) | SC-ENV-01 — VO map เปลี่ยน lighting ตามเวลา + weather effect/alert ตาม location ของ workspace owner (spec ถอดครบ 10 scenario จาก ClickUp + ผลตรวจ provider จริง + technical design ของ aggregator API) | **implement 11/17 PR (2026-09-10)** — Track A (api A1–A6 + `precip_pct`) + B (ws B1) ครบ, app C1+C2+C4+C6 เสร็จ (**แสงตามเวลา + widget + หน้าตั้งค่า owner ขึ้นจอแล้ว**) · asset ดวงจันทร์ 5 phase บน R2 · [preview ให้ทีมตรวจ](https://claude.ai/code/artifact/17865a33-4c40-445d-b84c-4e52a20c1ac6) · mig 98/99/100 บน dev · **PR draft: api#108 / ws#64 / app#336** (base develop, ยังไม่ merge) · เริ่มต่อที่ C5 หรือ C7 · **ยังไม่เคยยิง Google/TMD ด้วย key จริง** · รอ credential + มติ severity 2 ข้อ + asset 1× 3 ไฟล์ + คำตอบ design ข้อ 52/56 · เอกสาร: spec (ภาคผนวก A–F) · technical-design (§1–17) · task-breakdown · ux-ui-plan (§9 = weather panel) · progress |
...
| [\[Feature\] Nature Object Management](<[Feature] Nature Object Management — Object Management/>) | SC-OBJ-NAT-01 — เพิ่ม Object Type "Nature" (ต้นไม้/พุ่มไม้/ดอกไม้) ใน Object Management (Admin) เดิม — animated spritesheet ต่อ animation state, weather-reactive intensity, walkable เสมอ — spec + technical design (schema `tb_object_animation`, API contract, realtime broadcast) + ux-ui-plan (ถอด Figma ครบ 8 section) + test-plan + progress | **admin ใช้ asset จริงได้แล้ว — VO ยังไม่รองรับเฟรม** · 2026-09-23 · ClickUp [86d446fw1](https://app.clickup.com/t/86d446fw1) · schema + API + UI ตาม Figma ครบ + normalise spritesheet 6 เฟรมตอน upload · PR: [api#130](https://github.com/Maximumsoft-Co-LTD/zyra-api/pull/130) · [api#131](https://github.com/Maximumsoft-Co-LTD/zyra-api/pull/131) · [api#138](https://github.com/Maximumsoft-Co-LTD/zyra-api/pull/138) · [app#425](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/425) · [app#426](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/426) · [app#427](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/427) · [app#429](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/429) · [app#432](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/432) · [app#446](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/446) · **เหลือ:** frame-aware rendering ฝั่ง VO (`zyra-engine`) · เปิดหน้า admin ดูด้วยตา (ต้อง login) · §14.1 ข้อ 12 weather mapping รอ PM · แก้ spec ใน ClickUp 4 จุด |
| [LoadTest](LoadTest/) | Load test ทั้งระบบ (app/api/ws/notifications) — baseline จากโค้ด, safety constraints, load model ต่อ user, SC-LT-01 ~ 09 พร้อม pass criteria อ้าง capacity-scaling/tick-overhaul, gap/conflict · script อยู่ `zyra-loadtest` | **Draft spec — ยังไม่ approve** · 2026-09-24 · โครง `zyra-loadtest@feat/smoke-structure` (ยังไม่ commit) + smoke 1 endpoint ผ่านบน local · รอตอบ Q1 (ยิงที่ไหน) / Q3 (seed) |
...
| [Private-Zone-Claim](Private-Zone-Claim/) | จอง private zone ใน VO + in-place zone editor บน Pixi scene | plan.md: ClickUp `pending` · in-place editor รอบ 2 (local draft + batch commit) merge dev 2026-09-08, รอ live-verify |
...
| [Roadmap](Roadmap/) | Admin ของ Roadmap/Zyra Spotlight — จัดการฟีเจอร์บนไทม์ไลน์ + scene editor (วาง object, อัปโหลด sprite) | UI + API ต่อกันแล้ว 2026-09-17 · `zyra-api@feat/admin-roadmap-api` + `zyra-app@feat/admin-product-updates` (ยังไม่ commit) · ตารางขึ้น dev แล้ว · landing section 07 ต่อแล้วบน `zyra-landing@feat/roadmap-spotlight` |
...
| [TreeSway](TreeSway/) | ต้นไม้แกว่งตามลมใน VO (PixiJS) — pixel step, ความแรงตาม `wind_kph` ของ SC-ENV-01 · ตอนนี้ต้นเดียว (`Bigtree 5`) · แผนขยายหลายต้น (config รายต้นใน admin) | prototype เสร็จในโค้ด 2026-09-23 · `zyra-app@feat/tree-sway` ยังไม่ commit · ยังไม่ live-test |
