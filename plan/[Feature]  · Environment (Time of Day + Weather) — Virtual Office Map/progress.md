...
## รอบที่ 26 — 2026-09-22 · ขึ้น prod (v1.5.4) แล้วตามเก็บกรอบเหลี่ยม (v1.5.5)

**ขึ้น prod แล้ว** — [[v1.5.4]] (api+app) และ [[v1.5.5]] (app) · patch note ประกาศให้ผู้ใช้เห็นแล้ว · backfill ชื่อสถานที่บน prod วิ่งสำเร็จ **16/16 แถว · label ที่ไม่ใช่อักษรละติน 1 → 0**

**บทเรียนของรอบนี้ — asset ที่มี glow ต้องแยก "กรอบ" กับ "หน้าต่าง" ออกจากกัน**

| รอบ | ทำอะไร | ผล |
|---|---|---|
| 24 | กล่อง = ตัวเลขดีไซน์ตรง ๆ | เล็กไป 1/3 (margin 18% ในไฟล์กินที่) |
| 25 | ครอบ viewBox ที่กรอบ 40×32 | ขนาดเท่ากันแล้ว **แต่ตัด glow ขาด → สี่เหลี่ยมขอบคม** หลุดขึ้น prod |
| 26 | กรอบกำหนดสเกล · หน้าต่างขยายตาม glow ของไฟล์นั้น · ไทล์เป็นตัวตัด | ไม่มีกรอบ ขนาดยังเท่ากัน |

ที่พลาดคือรอบ 25 ตรวจด้วยการ render แล้ว**เห็นสี่เหลี่ยมนั้นอยู่** แต่สรุปว่าเป็นของในอาร์ตเวิร์กเอง (เพราะ thunder-storm ต้นฉบับก็มีเงาสี่เหลี่ยมจาง ๆ จริง) — ไม่ได้เทียบกับ artboard เดิมที่สเกลเดียวกัน ซึ่งพอเทียบแล้วตอบได้ใน 1 ภาพว่าไม่มี

**สองเรื่องที่เจอระหว่าง release และยังไม่ได้แก้** (รายละเอียดใน [[v1.5.4]]): OTP email บน prod ตั้งค่าว่าง · node เดียวจอง CPU 91% จน rollout ของ app ค้าง 15 นาที (ปลดด้วย `maxSurge: 0` ใน zyra-infra #43)

---

## รอบที่ 25 — 2026-09-22 · live-test บน dev แล้วพังสามจุด (รวมจุดที่กระทบ prod)

> รอบนี้คือรอบแรกที่ **เอาขึ้น dev แล้วดูของจริง** — และของจริงหักล้างข้อสรุปของรอบ 24 ไปสองข้อ

### 1. icon เล็กเกินไป แล้วยัง "ขนาดไม่เท่ากัน" ด้วย (app [#434](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/434) → [#435](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/435))

**อาการที่ user รายงาน:** "รูปมันเล็กมาก มองไม่เห็น" แล้วรอบถัดมา "ขนาดไม่สม่ำเสมอกัน"

| รอบ | เข้าใจว่า | ที่จริง |
|---|---|---|
| 24 | กล่อง = ตัวเลขใน design ตรง ๆ | ไฟล์มี margin 18% ⇒ เนื้อรูปเหลือ ~74% ของกล่อง **เล็กไป 1/3** (เมฆ 17px ที่ design วาด 20px · rain เหลือ ~11px) |
| 34 (#434) | คูณ 1.36 ชดเชย margin | แก้**ค่าเฉลี่ย**ได้ แต่ไม่ได้แก้**การกระจาย** — fit ตามขอบของแต่ละไฟล์ ⇒ ไฟล์ที่ core สัดส่วนต่างกันก็ยังออกมาคนละขนาด |
| 25 (#435) | — | ไป**ดึงกรอบจาก Figma** (`4635:148694`): ทุก variant วาดในกรอบ **40×32 เดียวกัน** ไฟล์ที่ส่งมาคือ artwork นั้น export 1× โดยมี blur ติดมา canvas เลยกว้าง 40×32 ถึง 72×112 |

**วิธีแก้จริง** — re-frame ทุกไฟล์ลงกรอบ 40×32 ร่วมกัน (×1.2 กัน glow โดนตัด) จัดกึ่งกลางที่ solid artwork · **ตรวจสอบได้:** 3 variant ที่ export เป็น 40×32 พอดี (cloudy/snowflake/wind ไม่มี blur ล้น) ตำแหน่งที่คำนวณได้ตรงกับ Figma คลาดไม่เกิน **1.5px** · path ไม่ถูกแตะ แก้แค่ `viewBox` + `width`/`height` บรรทัดแรก · กล่องในโค้ดจึงเป็นตัวเลข design ตรง ๆ ได้แล้ว (badge 24×19.2 · card 64×51)

**บทเรียน:** asset ที่ดีไซน์ export มาให้ ไม่ได้พก "กรอบ" ที่เขาจัดมาด้วย — ต้องไปเอากรอบจาก component set เอง ไม่งั้นเราจะจัดขนาดจากขอบ blur ซึ่งไม่ใช่ของที่คนมอง

### 2. ปุ่มลาก — มองไม่เห็น แล้วยังแย่งงานกับการเปิด panel

`:::` เดิมโผล่ตอน hover นอกกรอบป้าย ⇒ เป็น glyph สีเทาลอยอยู่บนแมพ **hover แล้วก็ยังไม่เห็น** · ระหว่างทางเจอว่ามันถูก tile บังครึ่งด้วย เพราะ `backdrop-blur` ของ tile สร้าง stacking context แล้ววาดทับ absolute sibling ที่ประกาศก่อนหน้า (ต้อง `z-10`)

ตอนนี้ย้ายเข้ามา**ในแถบ หน้าสุดก่อน tile เห็นตลอดเวลา** และ `VODraggable` รับ `handleSelector` ใหม่ ⇒ **ลากได้เฉพาะที่ `:::`** กดที่อื่น = เปิด panel อย่างเดียว (เดิมกดแล้วเผลอขยับ 4px กลายเป็นลากแทนที่จะเปิด) · double-click รีเซ็ตตำแหน่งย้ายไปที่ handle ด้วยเหตุผลเดียวกัน

### 3. ⚠️ `language=en` ของรอบ 24 **ไม่ได้แก้ปัญหาเลย** (api [#135](https://github.com/Maximumsoft-Co-LTD/zyra-api/pull/135))

ยิง Google จริงด้วยพิกัดของ member ที่ค้างเป็นไทย (`13.6977,100.3672`):

| พิกัดเดียวกัน key เดียวกัน `language=en` เหมือนกัน | ได้ |
|---|---|
| plain | `หนองแขม` |
| `result_type=sublocality\|locality\|administrative_area_level_2` | **`Nong Khaem`** |

`language` เลือกได้แค่จาก**ชื่อที่ Google มี** — เขตไทยหลายเขต ผลลัพธ์แบบ street address มีแต่ชื่อไทย ต้องถามเป็น **"พื้นที่"** ถึงจะได้ชื่อโรมัน · ทดสอบครบ 14 พิกัดใน dev: อังกฤษหมด ไม่มีอันไหนเปลี่ยนเขต · จุดที่ไม่มีพื้นที่ประเภทนี้ (กลางเกาะไอซ์แลนด์) ตอบ `ZERO_RESULTS` ⇒ fallback ไปถามแบบเดิม (ชื่อภาษาท้องถิ่นดีกว่าไม่มีชื่อ) · แถมเจอ Tokyo คืน `9` (เลข chome) เป็นชื่อสถานที่ ⇒ ข้าม component ที่ไม่มีตัวอักษรเลย

### 4. 🔴 poll loop ไม่เคยวิ่งบน dev — และ dev แย่ง lock จาก prod ได้

ไล่ว่าทำไม backfill ไม่ขยับสักแถว (ผ่านไป 3 tick ก็ยัง 0) เดาผิดไปหนึ่งรอบ ([#134](https://github.com/Maximumsoft-Co-LTD/zyra-api/pull/134) — คิดว่าโดน context 60s ร่วมกันแย่งไปหมด · แก้ให้มี budget ของตัวเองแล้วก็ยังไม่ขยับ) จนไปอ่าน Redis จริง:

```
env:poller:alerts   ttl=88   val=1
```

**dev / uat / prod ใช้ Memorystore ตัวเดียวกัน** (10.107.20.91) แต่ lock ของ poller เป็น key เปล่า ๆ ไม่มี prefix ⇒ **ล็อกเดียวใช้ร่วมกันทั้งสาม env** ใครคว้าได้ก่อนถือ 4m30s จากทุก 5 นาที ⇒ env ที่แพ้ถูกปิดเงียบทั้ง alert poll + weather refresh + backfill

**ไม่ใช่ปัญหาของ dev อย่างเดียว — dev แย่ง lock จาก prod ได้ แปลว่า prod อาจไม่ได้ poll เตือนภัยอยู่เป็นช่วง ๆ มาตลอด**

แก้โดย scope lock ด้วยชื่อ database (`env:poller:<db>:alerts`) — env ที่แชร์ Redis กันไม่เคยแชร์ DB กัน และ "หนึ่ง poller ต่อหนึ่ง database" คือเงื่อนไขที่ lock นี้มีไว้จริง ๆ · ของอื่นใน cache แชร์ได้ไม่เป็นไร (weather cell = อากาศเดียวกัน · signature ผูกกับ workspace id ที่มีอยู่ DB เดียว)

### Before/After — ชื่อสถานที่บน dev (วัดจริง)

| Metric | Before | After | Δ |
|---|---|---|---|
| label ที่ไม่ใช่อักษรละติน | 1 / 14 แถว | **0 / 14** | -100% |
| แถวที่ stamp ภาษาแล้ว | 0 / 14 | **14 / 14** | +14 |
| จำนวนวิธีสะกดของเขตเดียวกัน | 2 (`หนองแขม` / `Khet Nong Khaem`) | **1** (`Nong Khaem`) | -1 |
| เวลาที่ backfill ใช้ | ไม่เคยวิ่ง (3+ tick) | **tick แรกหลัง pod ขึ้น (~5 นาที)** | — |

**วัดยังไง**: `count(*) FILTER (WHERE env_place_label ~ '[^\x00-\x7F]')` และ `FILTER (WHERE env_place_lang='en')` บน `tb_workspace` + `tb_user`
**ช่วงเวลา**: before 15:31 (+07) ก่อน pod `dev-2d6bafb` ขึ้น · after 15:41 (+07) tick แรกหลังขึ้น
**แหล่งข้อมูล**: dev DB `zyra-db` โดยตรง · Redis prod ผ่าน `zyra-service/prod-db.sh` (อ่าน lock)

ชื่อทั้งหมดบน dev หลัง backfill: `Chatuchak` `Nong Khaem` `Tha It` `Thon Buri` `Min Buri` `Bang Kapi` — ทุกแถว `[en]`

### สถานะ

- ✅ **live-test บน dev ผ่านแล้ว** สำหรับ backfill + ชื่อสถานที่ (ตัวเลขข้างบนมาจาก dev จริง)
- ✅ `go test ./...` · `vitest` 185 ไฟล์ / 2527 เคส · `next build` · lint/prettier ผ่าน
- ⛔ **ป้ายกับ icon รอบล่าสุด ([#435](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/435)) ยังไม่ได้ live-test** — verify แค่ระดับ render 1:1 เทียบ Figma
- ⛔ **prod ยังไม่ได้แก้** — ยังเป็นโค้ดเก่า (1/16 label เป็นไทย · lock ยังชนกันข้าม env) ต้องรัน **migration 107** ก่อนปล่อย tag รอบหน้า
- ⚠️ ที่ควรตามต่อ: Redis ตัวเดียวถูกใช้ร่วมกัน 3 env — รอบนี้แก้เฉพาะ poller lock ส่วน key อื่น (presence, cooldown ฯลฯ) ยังไม่ได้ตรวจว่าชนกันหรือเปล่า
