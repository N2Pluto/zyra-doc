...
## 19. Phase ของดวงจันทร์ (asset 2026-09-10)

ได้ sprite มา 5 phase (ดู [ux-ui-plan §7.2.5](ux-ui-plan.md)) — เลือกใช้ตัวไหนคำนวณจากวันที่ **ไม่ต้องยิง API เลย** ซึ่งสำคัญเพราะ quota คือข้อจำกัดหลักของงานนี้ทั้งงาน

```
อายุดวงจันทร์ (วัน) = (JD ปัจจุบัน − 2451550.1) mod 29.530588853
```

`2451550.1` = เดือนดับ 6 ม.ค. 2000 · `29.530588853` = คาบ synodic แม่นพอสำหรับงานภาพ (คลาดไม่เกินไม่กี่ชั่วโมงในอีกหลายสิบปี) · คำนวณฝั่ง server คู่กับ `SunTimesFor` ที่มีอยู่แล้วใน `environment_sun.go` แล้วส่ง phase ไปกับ snapshot — client จะได้ไม่ต้องมีตารางดวงจันทร์ของตัวเอง

ยุบ 8 ช่วงลง 5 ไฟล์ที่มี (ถ้า PM ไม่ขอไฟล์เพิ่มตามข้อ 58):

| อายุ (วัน) | phase จริง | sprite ที่ใช้ |
|---|---|---|
| 0–1.8 | เดือนดับ | **ไม่วาดอะไร** (ท้องฟ้ามีแต่ดาว) |
| 1.8–9.2 | ข้างขึ้นเสี้ยว → ขึ้น 8 ค่ำ | `moon-waxing-crescent` → `moon-first-quarter` |
| 9.2–20.3 | ข้างขึ้นโป่ง → เต็มดวง → ข้างแรมโป่ง | `moon-full` |
| 20.3–27.7 | แรม 8 ค่ำ → ข้างแรมเสี้ยว | `moon-last-quarter` → `moon-waning-crescent` |
| 27.7–29.53 | เดือนดับ | **ไม่วาดอะไร** |

**ห้ามใช้ `moon/moon.gif` ตัวเดิม** — มันวนทั้ง 5 phase ทุก 2.5 วินาที (ข้อ 57)
**ซีกโลกใต้ต้องกลับด้าน sprite** เมื่อ `lat < 0` ไม่งั้นเสี้ยวจะหันผิดข้าง (ข้อ 59) — กลับด้านด้วย `scaleX(-1)` ได้เลย ไม่ต้องมีไฟล์เพิ่ม
