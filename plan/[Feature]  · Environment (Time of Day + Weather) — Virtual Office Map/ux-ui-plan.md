...
> **อัปเดต 2026-09-11 (C3 ขึ้น dev แล้ว):** องค์ประกอบท้องฟ้าที่วาดจริง **ยึดตาม [preview ที่ทีมรีวิว](https://claude.ai/code/artifact/17865a33-4c40-445d-b84c-4e52a20c1ac6) ไม่ใช่พิกัดในเฟรม Figma** — เฟรมเป็นภาพนิ่ง ลอกพิกัดตรง ๆ ได้ฝน 8 ช่องเรียงกันกับเมฆนิ่ง 6 ก้อน → ดู [§11](#11-สิ่งที่-c3-วาดจริง-2026-09-11) · ข้อ **65–68** เป็นสิ่งที่ตัดสินไปแล้วในรอบนี้ ทีมค้านได้
...
| **56** | ~~การ์ดที่ 2 **"Your weather"** ยังทำไม่ได้~~ **ปิดแล้วรอบที่ 22** — เดิม: ยังทำไม่ได้ — ผูกกับโมเดล "My location" ที่ยังไม่ตัดสิน (ข้อ 42/45) | ทำแค่การ์ด workspace · `WeatherCard` รับ label เป็น prop ไว้แล้ว เพิ่มการ์ดที่ 2 = เพิ่ม call site เดียว |
...
## 10. Personal preference (C7 / HP-05) — ถอดจริง 2026-09-10

**node:** section `4839:34795` → instance **`4839:38016` "Environment setting - Member"** (+ อีก 6 เฟรมที่เป็น instance ตัวเดียวกัน) · เฟรมแมพ `5652:69883` (แสงเวลา) · `5746:35711` (พายุ) · `5732:212550` (ปิดหมด)

**ที่สำคัญที่สุด: HP-05 ไม่ใช่หน้าใหม่** — มันคือ **member variant ของ Setting → Environment tab เดียวกับ C6** ⇒ tab นี้เลิกเป็น owner-only แล้ว (แก้มติของ C6) และสลับ**แถว**ตาม role ไม่ใช่ซ่อนทั้ง tab

| ส่วน | owner (`4635:150663`) | member (`4839:38016`) |
|---|---|---|
| การ์ด Workspace weather | ✅ | ✅ (เหมือนกัน) |
| Workspace location / Weather alert notification | ✅ | ❌ |
| My location + การ์ด "Your weather" | ❌ | ✅ **ทำแล้ว (รอบที่ 22)** — node `4872:592375` |
| Time of day lighting / Weather visual effects | ✅ (workspace-wide) | ✅ (**ส่วนตัว**) |

ข้อความทุกแถวตรงกับ key ที่มีอยู่แล้ว (`envRowTodTitle/Desc`, `envRowEffectsTitle/Desc`) — design ใช้คำเดียวกันทั้งสอง role จึง reuse ไม่เพิ่ม key

### 10.1 sticky 3 ใบที่กำหนดพฤติกรรมบนแมพ (สำคัญกว่าตัว UI)

| node | ข้อความ | แปลว่า |
|---|---|---|
| `5529:990728` | "Shade & Shadow เวลา แสงกระทบต่อแมพ" | แสงตามเวลา = C2 ที่ทำแล้ว |
| `5732:212544` | "ถ้ามีการปิดสภาพอากาศจะไม่ปรากฎ สภาพอากาศนั้น**+แสงของสภาพอากาศ** ... พอปิดก็ยังมีแสงไฮไลท์แสดงว่าเป็นกลางวันอยู่" | ปิด weather = particle **และ** tint ของอากาศหายทั้งคู่ แต่ **แสงตามเวลายังอยู่** ⇒ สองสวิตช์เป็นอิสระต่อกันจริง |
| `5732:212734` | "จะไม่มีการโชว์แสงของเวลา + สภาพอากาศ" | ปิดทั้งคู่ = ไม่วาด layer อะไรเลย |

### 10.2 conflict ใหม่

| # | เรื่อง | ทำอะไรไป |
|---|---|---|
| **60** | **spec บอกปิด time-of-day แล้วให้ fallback เป็น Morning** (AC ของ HP-05) แต่ sticky `5732:212734` บอกว่า**ไม่โชว์แสงเลย** | ทำตาม design — ไม่วาด layer · บนจอผลลัพธ์เท่ากันอยู่แล้วเพราะ morning = alpha 0 และ "ไม่มี layer" คือสิ่งที่ทำให้ AC ข้อ performance ("ปิดแล้ว FPS ขึ้น") เป็นจริง |
| **61** | **ไม่มีปุ่ม "ปิดทั้งหมด"** ที่ spec วาดไว้ใน layout ของ HP-05 | ไม่ทำ — design ตัดออก และมีสวิตช์แค่ 2 ตัว ปุ่มรวมจึงประหยัดได้แค่ 1 คลิก · **ถ้า PM ยืนยันว่าต้องมี ให้แจ้ง** |
| **62** | **"My location" + การ์ด "Your weather"** อยู่ในเฟรม member เต็ม ๆ แต่ต้องมี location ราย user ที่ API ยังไม่มี และติดมติ PII (ข้อ 42/45) | ~~ไม่ทำในรอบนี้~~ → **ทำแล้วรอบที่ 22** ตามที่ผู้ใช้สั่ง · location ราย user อยู่บน `tb_user` (migration 101) · เขียนผ่าน `PUT /api/user/workspaces/:id/environment/me` · **PII (ข้อ 45) ยังเปิดอยู่** — ผู้ใช้สั่งให้ทำ จึงบันทึกไว้ว่ารับความเสี่ยงนี้แล้ว (เก็บเฉพาะตอนกดเปิดเอง · เห็นได้เฉพาะเจ้าของแถว · ปิดแล้วหยุดใช้ทันที) |
| **63** | design ไม่มีสถานะ **disabled** ของ 2 แถวส่วนตัว ตอนที่ workspace ปิดเอฟเฟกต์นั้นอยู่ | ทำ disabled แบบเดียวกับแถวของ owner ตอน location ปิด (pattern เดิมใน tab นี้) — เพราะ preference **ลบได้อย่างเดียว** ปล่อยให้กดได้จะเป็นสวิตช์ที่กดแล้วไม่มีอะไรเกิดขึ้น · **ค่าที่เก็บไว้ไม่ถูกแตะ** คนที่ปิดไว้ยังปิดอยู่เมื่อ owner เปิดกลับ |
| **64** | owner ไม่มีแถวส่วนตัวเลยใน design | ทำตาม design — owner ปิดเอฟเฟกต์ของตัวเองผ่านสวิตช์ workspace ซึ่งปิดให้จอตัวเองด้วยอยู่แล้ว |
...
## 11. สิ่งที่ C3 วาดจริง (2026-09-11)

ทำแล้วและอยู่บน `develop` — [#347](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/347) + [#350](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/350) · ตาราง layer ทั้ง 323 ชิ้นอยู่ที่ `lib/environment-fx-composition.ts` (gen จาก payload ของ preview ด้วย `scripts/gen-environment-fx.py` — ถ้า preview เปลี่ยน ให้ re-gen ไม่ใช่แก้มือ)

### 11.1 layer model ที่ใช้จริง — ต่างจาก §8 สองแถว

| z | เลเยอร์ | ทำจริงเป็นแบบไหน |
|---|---|---|
| 0 | ฉากหลังนอกแมพ | `bg/zyra-backdrop-2x.png` (2560×1440 ตัวเต็ม) · **เฟรมให้พอดีกล่องของดีไซน์แล้วขยายจนครอบทุกจุดที่กล้องไปถึง** เส้นขอบฟ้าตรึงเหนือขอบบนแมพ ⇒ แมพ "ตั้งอยู่บน" พื้นหญ้า |
| 1 | วัตถุบนท้องฟ้า | ดวงอาทิตย์/พระจันทร์ **ชิ้นเดียว เดินบนส่วนโค้ง** (ขึ้น-ตกตามพิกัดจริง ตัดที่เส้นขอบฟ้า) · เมฆ/ดาว/ฟ้าผ่า ปูซ้ำ**แนวนอนเท่านั้น** อยู่ในแถบฟ้าเหนือแมพ |
| 2 | แมพ | ของเดิม ไม่แตะ |
| 3 | เอฟเฟกต์อากาศ | **ไม่ใช่ "ทับทั้ง viewport"** — อยู่ในโลก ปูซ้ำทั้งแนวนอนและแนวตั้งจนคลุมทั้งแมพ วาด**หน้า**แมพ (ไม่ใช่หลัง) และไม่รับ pointer event |
| 4 | แผ่นสีตามช่วงเวลา | ทับทั้ง viewport ตามเดิม · **ไล่ค่าต่อเนื่อง**จากนาฬิกาเดียวกับดวงอาทิตย์ ไม่กระโดดระหว่าง 5 ค่า |
| 5 | HUD / widget / banner | ของเดิม |

### 11.2 สิ่งที่ตัดสินไปแล้วในรอบนี้ (ทีมค้านได้)

| # | เรื่อง | ทำอะไรไป |
|---|---|---|
| **65** | **องค์ประกอบมาจาก preview ไม่ใช่เฟรม Figma** — จำนวนชิ้น ตำแหน่ง ความทึบ ความเร็วลอย | ทำตาม preview ทั้งชุด: ฝน 56 เม็ด 3 ขนาด · เมฆมาก 19 ก้อน 3 ระยะ · ทุกชิ้นลอยขวา→ซ้ายคนละรอบคนละความเร็ว · **ถ้าทีมอยากได้ตามเฟรมจริง ๆ ให้แจ้ง** |
| **66** | **ไม่มี wash สีเทา/น้ำเงินต่อ condition แบบที่ HP-03 ระบุเป็นค่า rgba** | ตาม preview — เหลือ wash เฉพาะ **fog** กับ **thunderstorm** (+ ม่วงของกลางคืน) ส่วน rain/cloudy ได้น้ำหนักจาก sprite + แสงตามเวลาแทน |
| **67** | **`snow` / `windy` วาดแล้ว** ทั้งที่ข้อ 11/29 ยังไม่มีมติ | วาดตาม preview ไปก่อน · ไฟล์ยังเป็นต้นฉบับที่ใหญ่ (decode 12.3 / 8.6 / 6.7 MB) **ยังต้องขอ 1× จากดีไซน์** ([§7.2.1](#721-⚠️-ต้องขอไฟล์ต้นฉบับขนาด-1×-ก่อนนำไปใช้)) |
| **68** | **ไล่สีท้องฟ้า + แสงประกายรอบดวงอาทิตย์ (glow)** เป็นของที่ preview เพิ่มเอง ไม่มีใน spec/Figma | ใส่ glow แล้ว (อยู่ใต้ sprite เมฆจึงบังได้) · ไล่สีท้องฟ้าแบบ gradient **ไม่ได้ใส่** — backdrop ของจริงมีท้องฟ้าอยู่แล้ว |

### 11.3 ยังไม่ได้เห็นด้วยตา

ฝน · หิมะ · ลม · ดวงอาทิตย์ตอนกลางวัน + glow · พระจันทร์ — สภาพอากาศจริงของพิกัดทดสอบเป็น cloudy/drizzle ตลอดช่วงที่ทำ · ที่ยืนยันด้วยตาแล้วคือ เมฆ · backdrop · แสงกลางคืน · **และ AC เรื่อง FPS ยังไม่ได้วัด**
