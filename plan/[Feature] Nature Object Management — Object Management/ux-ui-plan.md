...
> **🔄 ClickUp รอบที่ 3 (2026-09-17 ~10:36):** status ทุก task → **`in progress`** แต่ **description/comment ไม่เปลี่ยนเลย** → **§14.1 ทั้ง 12 ข้อที่เปิดอยู่ ยังเปิดเหมือนเดิม** (รวมข้อ 14a สี/ประเภท box) — ต้องเคลียร์กับ PM ก่อนเริ่ม UI ที่ผูกกับ schema · ดู [spec.md §รอบที่ 3](spec.md#รอบที่-3--2026-09-17-status-เปลี่ยนเป็น-in-progress) · test ที่อิง §14.1 ห้ามล็อกค่า — ดู [test-plan.md §0](test-plan.md#0-สิ่งที่ห้ามล็อกค่าใน-test-จนกว่า-pm-จะเคาะ)
...
|---|---|---|---|---|
| 1 | **Required states** | big/pine/bamboo/shedding = idle+sway_light+sway_strong · bush/flower = idle+sway_light · falling optional | **Idle เท่านั้น** (PM sticky ×2) · ไม่มี Required/Optional marker · Save enable หลัง Idle ตัวเดียว | `NatureRequiredStates` ใน technical-design §4 + active-gating §6.4 ต้องแก้ตาม |
| 2 | **ชื่อ state ที่ 3** | `sway_strong` | **`Sway normal`** | key ↔ label: เก็บ `sway_strong` แล้ว label "Sway normal"? หรือเปลี่ยน key เป็น `sway_normal`? |
| 3 | **frame_count / frame_rate** | กรอกต่อ state (1–64 / 4–24) ใน Animation Manager | ไม่มีใน modal · dropdown read-only ระดับ object บนฟอร์มหลัก (`50`/`24`) · PM: "อนาคต ยังไม่มีให้กรอก" | technical-design §5.2 รับจาก multipart — client ส่งอะไร? derive จาก image? เก็บระดับ object ไม่ใช่ต่อ state? |
...
| 5 | **Nature menu / entry point** | menu "Nature" ข้างเมนูอื่น | เป็น category ที่ 10 ใน Object Management (filter checkbox + dropdown + badge) | ไม่ต้องมี route/tab ใหม่ · `GET /api/admin/objects?types=nature` พอ |
...
| 7 | **`custom` nature_type** | 7 ตัวรวม custom | **6 ตัว** ไม่มี Custom | ตัด `custom` ออกจาก enum/validation หรือคงไว้ฝั่ง API แต่ไม่โชว์ |
| 8 | **Status default** | Hidden (ต้อง upload ก่อน Active) | toggle **ON = Active** ตั้งแต่ Creating · ไม่มี gating/tooltip บน toggle (gating อยู่ที่ Save) | technical-design §5.1 `status=active` reject → ต้องเปลี่ยนเป็น gating ที่ Save (Save disabled จนกว่ามี Idle) |
| 9 | **Delete: พิมพ์ชื่อยืนยัน** | เฉพาะ `placed_objects_count > 0` | **ทั้ง 2 เงื่อนไข** (= code เดิม) · field ที่มีคือ `workspace_count`/`map_count` | คง behaviour code เดิม (ตรง Figma) — spec ต้องอัปเดต |
| 10 | **Transparency check** | warning เหลือง ไม่บล็อก | **error แดง บล็อก** | validation rule ฝั่ง client+server |
| 11 | **Preview controls** | state buttons · Current State label + reason · playback pause/step/speed 0.25x–2x · wind 0–100 | **ไม่มีเลย** — weather dropdown 5 ตัว + wind slider (scale ไม่ linear, max `ต้องดึง`) + zoom | **ยังเปิด (รอบที่ 2)** — PM ลบ ASCII mock ออกแล้วแต่ AC ทั้ง 3 ข้อยังอยู่ · **AC ใหม่:** state ที่ยังไม่ upload → preview แสดงเป็น **idle** (ต้องทำ fallback ใน preview logic) · ตัด/ขอ Figma เพิ่ม |
| 12 | **Weather 5 vs 4 + mapping** | Clear/Cloudy/Rain/Storm | Clear/Cloudy/Rain/**Strong rain**/**Thunderstorm** · PM HP-01: Idle:Clear · Sway light:Cloudy · Sway normal:Rain · Falling: shedding เท่านั้น → Strong rain/Thunderstorm map ไป state ไหน? | preview logic + default wind ต่อ weather (Clear 9? Rain 29 ≠ ≥30) |
| 13 | **"(Hidden)" badge ใน Map Editor** | มี | ไม่มีทั้ง Figma และ code | ตัด หรือออกแบบเพิ่ม |
| 14 | **Name max length** | 100 | `/100` | code `MAX_NAME_LENGTH = 50` → ขยายเป็น 100 กระทบทุก type (DB `VARCHAR(100)` รองรับอยู่แล้ว) |
| **14a** (ใหม่ รอบที่ 2) | **สี/ประเภทของ box object หลังบันทึก** | HP-03 AC ใหม่: "หลังบันทึกสามารถกำหนดสีและประเภทของ box object ได้" **แต่** HP-02: "Collision: fixed = walkable ไม่แสดงใน form" | Object Preview หลัง save มี hitbox toolbar (สี · brush blocked/walkable · eraser) + colour dot ที่แถว Object files (§4.4 / §7.1) | ถ้า "ประเภท" = hitbox blocked/walkable → Nature **ต้องมี `object_compositions` row** → ขัด technical-design §3 (ออกแบบว่าไม่มี row = walkable อัตโนมัติ) · ถ้า "ประเภท" หมายถึงอย่างอื่น (เช่น tag/variant) ต้องให้ PM นิยาม · **ต้องเคาะก่อนทำ schema** |
...
| Zoom | track/fill/knob สี, min/max zoom | `1568:301785` |
| Slider ลม | min/max/step · snap band ไหม · hover/active/disabled · default ต่อ weather (ตาราง) | ถาม designer/PM |
| Weather | selected/hover row · chevron หมุน · mapping Strong rain/Thunderstorm → state · icon custom vs lucide | ถาม designer |
...
| Header buttons | disabled สีของปุ่ม 42px (Preview/Save) · Delete อยู่ไหน | `5219:761386` / ถาม designer |
