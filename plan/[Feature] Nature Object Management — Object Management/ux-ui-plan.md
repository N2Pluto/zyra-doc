...
| ~~3~~ | ~~**frame_count / frame_rate**~~ | กรอกต่อ state | dropdown read-only ระดับ object | **✅ ปิดแล้ว 2026-09-20 · 🔄 แก้บางส่วน 2026-09-23** — ยังไม่มีช่องให้กรอก (มติเดิมคงอยู่) แต่เหตุผลเดิม "ภาพนิ่ง interim = `frame_count 1` พอดี" **ใช้ไม่ได้แล้ว**: asset จริงเป็น strip 6 เฟรม → `frame_count` **ตรวจจากชีตเอง** ตอน upload (ดู [technical-design §5.2.2](technical-design.md#522--asset-จริงมาแล้ว-2026-09-23--normalise-spritesheet-ตอน-upload)) และฟอร์มโชว์ค่าจริงของ Idle แทนเลข `1` ที่ hardcode ไว้ · `frame_rate` ยังเป็น DB default `12` เพราะยังไม่มีแหล่งค่าอื่น |
