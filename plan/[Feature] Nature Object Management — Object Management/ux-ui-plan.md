...
> **✅ Implement แล้ว 2026-09-22 ([zyra-app#429](https://github.com/Maximumsoft-Co-LTD/zyra-app/pull/429), บน dev แล้ว):** HP-01 tree card · HP-02 create form · HP-03 upload modal (`nature-upload-modal.tsx`) · HP-06 saved/edit · EP-01 toasts (`nature-upload-validation.ts`) · EC-01 replace (`replace-spritesheet-dialog.tsx`) · HP-07 ใช้ของเดิมที่ตรงอยู่แล้ว · **ยังไม่ทำ:** HP-05 preview modal (`5196:368373`) — ปุ่ม Preview ใน modal จึง disabled ไว้ · **ตั้งใจต่างจาก design 2 จุด:** Frame count/rate โชว์ `1`/`12` (ค่าที่ API เก็บจริง) ไม่ใช่ `50`/`24` ของ mock · HP-06 ยังมีปุ่ม Delete (§14.3 ข้อ 28 — Figma ขัดกันเอง ถ้าเอาออกจะลบ object ไม่ได้เลย) · **ยังไม่ verify ด้วยตา** — ดู [progress.md รอบที่ 9](progress.md)
