...
> **🔄 อัปเดต 2026-09-23 — asset จริงมาแล้ว สมมติฐาน "ภาพนิ่ง 1 เฟรม" ตายแล้ว** ดู [§5.2.2](#522--asset-จริงมาแล้ว-2026-09-23--normalise-spritesheet-ตอน-upload) · ข้อความด้านล่างเก็บไว้เป็นบันทึกว่าตอนนั้นตัดสินใจอะไรบนข้อมูลชุดไหน — **ข้อ 1 กับ 2 ใน "สิ่งที่ต้องระวัง" ถูกแทนที่แล้ว**
...
#### 5.2.2 ✅ Asset จริงมาแล้ว (2026-09-23) — normalise spritesheet ตอน upload

ไฟล์ชุดแรกจากคนทำ asset: `Tree Animation.png` + `Tree idle.png` — **วัดของจริงแล้วไม่ตรงกับที่บอกกันไว้**

| | ที่บอกไว้ | **ของจริงที่วัดได้** |
|---|---|---|
| ขนาดชีต | 1000×1000 | 1000×1000 ✓ |
| ขนาดเฟรม | 160×220 | **160×222** (`Tree idle.png` เป็นขนาดนี้เป๊ะ) |
| ระยะห่างต่อเซลล์บนชีต | 160 | **1000/6 = 166.67** |
| จำนวนเฟรม | — | **6** |
| แถวที่มีภาพ | — | **16–237** เท่านั้น (ข้างล่างว่าง 778 แถว) |

→ **ถ้าหั่นที่ 160 เป๊ะ จะกินเข้าไปในเฟรมเรื่อย ๆ** (เฟรมที่ 6 อยู่ที่ x 837–996 แต่ 5×160 = 800) · ที่ถูกคือ **ภาพวาด 160 กว้าง แต่ export วางห่างกัน 166.67** ซึ่งเป็นคนละเรื่องกัน

**ปัญหาที่ทำให้ต้องแก้:** โค้ด interim เก็บชีตดิบ ๆ ด้วย `frame_count = 1`, `frame_width/height = 1000×1000` ซึ่ง**ไม่จริง** → ทุกที่ที่วาดสไปรต์เลยวาดทั้งชีต = เห็นต้นไม้ 6 ต้นเบียดกันอยู่มุมกล่อง

**ที่ทำ — normalise ตอน upload ครั้งเดียว** (`zyra-api/internal/service/nature_sprite_strip.go`) แทนที่จะให้ทุก consumer รู้เรื่อง padding กับ pitch เศษส่วนเอง:

1. หา strip จาก alpha channel — บล็อกคอลัมน์ทึบที่ต่อเนื่องกัน และแต่ละบล็อกต้องอยู่ในเซลล์ความกว้างเท่ากันของตัวเอง (tolerance 2px)
2. crop เอาเฉพาะแถวที่มีภาพ แล้วเขียนใหม่เป็น strip ชิด ๆ N เซลล์เท่ากัน
3. เก็บ `frame_count` / `frame_width` / `frame_height` ของ **strip ที่เขียนใหม่** ไม่ใช่ของไฟล์ที่อัปมา

ผลกับไฟล์จริง: `Tree Animation.png` → **6 เฟรม 167×222** (ชีต 1002×222, 164 KB → 157 KB) · `Tree idle.png` → **1 เฟรม 160×222 ไบต์เดิมไม่แตะ**

**กติกาที่ห้ามพัง:**

- **เฟรมต้องคงตำแหน่งแนวนอนภายในเซลล์ของตัวเอง** — ระยะเยื้องนั่นแหละคือ animation (ต้นไม้เอนซ้ายขวาในรอยเท้าเดิม) ถ้า "จัดกลางให้" = ลบ animation ทิ้ง
- **copy เซลล์ ห้าม resample** — ไม่งั้นภาพ pixel art เบลอ และลำต้นกระตุกทีละพิกเซลระหว่างเฟรม
- ชีตที่เป็นเฟรมเดียวชิดขอบอยู่แล้ว → คืนไบต์เดิม ไม่ decode/encode ใหม่

**ข้อจำกัดที่รู้ตัว (มีเทสตั้งชื่อกำกับไว้ ห้ามมา "แก้" ด้วยการเดา):** ถ้า export แบบ**ไม่มีช่องว่างใสคั่นระหว่างเฟรม** คอลัมน์ทึบจะต่อกันเป็นก้อนเดียว แยกจากภาพเดี่ยวกว้าง ๆ ไม่ออกจาก alpha → fallback เป็น 1 เฟรม (= พฤติกรรมเดิม ไม่ regress) · asset จริงทั้ง 2 ไฟล์มีช่องว่าง 3–6px

**ฝั่ง client** (`zyra-app`): `NatureSpriteFrame` วาดทีละเซลล์ผ่าน `background-position` + `image-rendering: pixelated` · ไฟล์ที่ staged ไว้ (ยังไม่อัป) ตรวจ layout เองตอนเลือกไฟล์ด้วย algorithm เดียวกัน เพราะยังเป็น export ดิบที่มี padding อยู่ — **สอง implementation นี้ต้องตรงกันเสมอ** ถ้าไม่ตรง client จะ preview ที่ pitch ที่ server ไม่ได้เก็บ

**ที่ยังค้าง:** ฝั่ง VO/Phaser/Pixi ยังวาดสไปรต์แบบภาพเดียวอยู่ — ยังไม่ได้แตะในรอบนี้ ([§5.2.1](#521--interim-2026-09-17--รับสไปรต์ขนาดใหญ่ถึง-1000px-ระหว่างที่-asset-ยังไม่เสร็จ) ข้อ 2 เรื่อง contain scaling ยังใช้อยู่ที่นั่น) → ถ้าเอา nature ขึ้น map จริงต้องทำต่อ
