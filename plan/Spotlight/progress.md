...
- **ถึงไหน:** zyra-ws `fix/spotlight-start-tolerant-tile` — `zoneTypeClaimTileOK` (reconcile + leg END) แทน check ดิบ + `slog.Warn` ตอน reject ไว้วัด · zyra-app `fix/spotlight-start-confirmed-state` — `confirmed = self ∈ speakers && !disconnected`, pending/ack-timeout 1500ms, retry 500ms บน transient reject, budget 3 ส่ง, error handler + toast + Play กลับมา, `arrived` gate จาก `liveZoneId` (ปุ่ม disabled จนถึงจริง — ตามที่ผู้ใช้เลือก), publisher room เปิดหลัง confirm เท่านั้น, `welcome` bump epoch แทน re-assert listener เดิม · i18n en/th 5 key · โค้ดเสร็จทั้งสอง repo **ยังไม่ commit/push/PR**
- **PR:** ยังไม่เปิด
...
- **ต่อจากนี้:** commit + PR ทั้งสอง repo (ws ก่อน) → merge develop → live-test บน dev ตาม checklist ในไฟล์ issue §รอบที่ 2 → วัด `spotlight start rejected` rate จาก Loki 24h ก่อน/หลัง → ค่อย main/tag (ถาม confirm)
