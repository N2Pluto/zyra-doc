...
| C→S | **`POST /api/user/maps/:mapId/objects/batch`** | `{ zone_id, creates[], moves[], metas[], deletes[] }` → `{ created: [{client_id, object}], updated[], deleted[] }` | SC-PZ-05/06 — commit ทั้ง session ใน 1 transaction (เพิ่ม 2026-09-08, zyra-api #100/#101) · validate ครั้งเดียวต่อ batch ด้วยกฎเดิมทุกข้อ · budget/cap คิดแบบ **net** จึงสลับเฟอร์นิเจอร์ตอนเต็ม limit ได้ |
...
- **Write mode = local draft + batch commit (เปลี่ยนเมื่อ 2026-09-08, user confirm)** — ทุก action แก้ **draft ในเครื่องเท่านั้น** ไม่ยิง request: sprite โผล่เฟรมเดียวกัน · กด **Done** → `commitZoneObjectBatch` ยิง `POST /api/user/maps/:mapId/objects/batch` ครั้งเดียว 1 transaction → republish obstacle grid 1 ครั้ง + broadcast `map_updated` 1 ครั้ง · กด **Cancel** → คืน snapshot ก่อน edit ด้วย sprite delta ไม่มี request เลย (มี confirm ก่อนเพราะตอนนี้ Cancel ทิ้งงานได้จริง) · undo/redo local ล้วน
  - payload สร้างจากการ **diff draft กับ snapshot** ไม่ใช่ replay undo stack — วางแล้วลบ / ย้ายแล้วย้ายกลับ / หมุนครบรอบ จึงไม่ส่งอะไรเลย (`views/user/virtual-office/utils/pz-draft.ts`)
  - commit เป็น **all-or-nothing**: item ใดไม่ผ่าน → 400 + `item: {kind, ref}` ระบุชิ้นที่ผิด, ไม่เขียนอะไรเลย, draft ยังอยู่ให้ผู้ใช้แก้
  - **เหตุผลที่เลิก write-through**: วาง 1 ชิ้น = 1 request ที่ backend คิด ~10 query + scan `tb_map_object` ทั้งแมพ 3 รอบ และ sprite ไม่ขึ้นจนกว่า `map_object_changed` echo จะกลับมา (ghost ถูกลบตอน drop แล้ว → เห็นช่องว่างเปล่า) · move มีถึง 3 round-trip เรียงกัน · Clear all / Cancel เป็น loop `await` ทีละชิ้น
  - **ยังใช้ `use-editor-write-buffer.ts` ไม่ได้ตรง ๆ** — draft ที่นี่ต้องเก็บ `MapObject` เต็มรูปเพื่อให้ renderer/hit-test/occupancy index/ตัวนับ x/20 ใช้ร่วมกันได้ · ใช้ pattern เดียวกัน (temp id + flush on Save + remap) แต่ diff-based
  - endpoint เดิมทั้ง 5 ตัวยังอยู่ — admin Space Builder ยังใช้
...
| T08 | SC-PZ-05 วาง object (draft ส่วนตัว) | เปิด Zone Editor → ลากวาง (browser B เปิดดูอยู่)                    | snap grid, footprint เขียว/แดง, ไม่มี wall ใน library, **A เห็น sprite ทันที + ไม่มี request เลย** · **B ไม่เห็นอะไรจนกว่า A กด Done** · กด Done → B เห็นทั้งชุดพร้อมกัน · กด Cancel → กลับสภาพเดิม ไม่มี request (**เปลี่ยนจาก live-per-action เมื่อ 2026-09-08 — ดู §Write mode**) |
